...
export const ResultsView: React.FC<ResultsViewProps> = ({
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-10">
      <div
        className={`bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden transition-colors duration-500 ${
          isLive ? "ring-2 ring-indigo-50 border-indigo-100" : ""
        }`}
      >
        <div
          className={`p-8 text-white text-center transition-colors duration-500 ${
            isLive ? "bg-indigo-600" : "bg-slate-900"
          }`}
        >
          <div
            className={`inline-flex items-center justify-center p-3 rounded-full mb-4 ${
              isLive ? "bg-white/20" : "bg-yellow-500/20"
            }`}
          >
            {isLive ? (
        ...
        <div className="p-8 bg-slate-50/50">
          ...
          {totalVotes === 0 ? (
            ...
            <div className="flex flex-col md:flex-row items-end justify-center gap-4 md:gap-8 mb-12 min-h-[300px]">
              ...
              {(window.innerWidth < 768 ? top3 : podiumOrder).map(
                (result, index) => {
                  ...
                  if (isFirst) {
                    ...
                    colorClass =
                      "bg-gradient-to-b from-yellow-50 to-white border-yellow-200 shadow-yellow-100";
                    rankLabel = "1st Place";
                    icon = (
                    ...
                    colorClass =
                      "bg-gradient-to-b from-slate-50 to-white border-slate-300 shadow-slate-200";
                    rankLabel = "2nd Place";
                    icon = (
                    ...
                    colorClass =
                      "bg-gradient-to-b from-orange-50 to-white border-orange-200 shadow-orange-100";
                    rankLabel = "3rd Place";
                    icon = (
                      <Medal className="w-6 h-6 text-orange-400 fill-orange-300" />
                    );
                  }
                  ...
                  return (
                    ...
                    >
                      {/* Floating Image */}
                      <div
                        className={`absolute -top-12 md:-top-16 perspective-container ${
                          isFirst
                      ...
                      >
                        <div
                          className={`card-3d-wrapper w-full h-full rounded-full border-4 shadow-md bg-white ${
                            isFirst
                        ...
                        {isFirst && (
                          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-yellow-400 rounded-full blur-2xl opacity-20 -z-10 animate-pulse"></div>
                        )}
              ...
              return (
                <div
                  ...
                  className={`flex flex-col bg-white rounded-xl border ${
                    isWinner
                      ? "border-yellow-300 ring-1 ring-yellow-200 shadow-yellow-100"
                      : isTieBreakerWinner
                ...
                >
                  <div
                    className={`px-4 py-3 border-b ${
                      isWinner ? "bg-yellow-50/50" : "bg-slate-50/50"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        {/* {isTieBreakerWinner && (
                          ...
                          //   <Star className="w-2 h-2 fill-indigo-600" />
                          // </span>
                        )} */}
                      </div>
                    </div>
                    ...
                    {entry.voters.length > 0 ? (
                      <div className="flex flex-wrap gap-1.5">
                        {entry.voters.map((voter, idx) => {
                          const isGuest =
                            VOTERS.find((v) => v.id === voter)?.type ===
                            VoterType.GUESS;
                          return (
                            <span
                              key={idx}
                              className={`inline-block px-2 py-0.5 border rounded text-[10px] font-medium ${
                                isGuest
